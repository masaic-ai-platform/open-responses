...
class EvalRunnerTest {
    ...
    @Test
    fun `processEvalRun should update status to IN_PROGRESS when started`() = runTest {
        // Arrange
        ...
        // Assert
        coVerify(exactly = 1) { evalRunRepository.updateEvalRun(match { 
            it.id == evalRun.id && it.status == EvalRunStatus.IN_PROGRESS 
        }) }
    }
    
    @Test
    fun `processEvalRun should handle missing evaluation and set FAILED status`() = runTest {
        // Arrange
        val evalRun = createSampleEvalRun()
        ...
        coVerify { 
            evalRunRepository.updateEvalRun(match { 
                it.id == evalRun.id && 
    ...
    @Test
    fun `processEvalRun should handle missing data source processor and set FAILED status`() = runTest {
        // Arrange
        val evalRun = createSampleEvalRun()
    ...
    @Test
    fun `processEvalRun should handle error in loadDataLines and set FAILED status`() = runTest {
        // Arrange
        val evalRun = createSampleEvalRun()
        val eval = createSampleEval(evalRun.evalId)
    ...
    @Test
    fun `processEvalRun should handle error in processDataSource and set FAILED status`() = runTest {
        // Arrange
        val evalRun = createSampleEvalRun()
        val eval = createSampleEval(evalRun.evalId)
    ...
    @Test
    fun `processEvalRun should handle EmptyProcessingResult and set FAILED status`() = runTest {
        // Arrange
        val evalRun = createSampleEvalRun()
        val eval = createSampleEval(evalRun.evalId)
    ...
    @Test
    fun `processEvalRun should handle JsonlDataResult and set FAILED status as not implemented`() = runTest {
        // Arrange
        val evalRun = createSampleEvalRun()
        val eval = createSampleEval(evalRun.evalId)
    ...
    @Test
    fun `processEvalRun should handle CompletionMessagesResult with invalid data source type`() = runTest {
        // Arrange
        // Create a custom RunDataSource that is not a CompletionsRunDataSource
        val customDataSource = object : RunDataSource {
    ...
    @Test
    fun `processEvalRun should handle successful completion message processing`() = runTest {
        // Arrange
        val evalRun = createSampleEvalRun()
        val eval = createSampleEval(evalRun.evalId)
        val completionMessages = listOf(ChatMessage("user", "text"))
    ...
    @Test
    fun `processEvalRun should handle generation service errors`() = runTest {
        // Arrange
        val evalRun = createSampleEvalRun()
        val eval = createSampleEval(evalRun.evalId)
        val completionMessages = listOf(ChatMessage("user", "text"))
    ...
    @Test
    fun `processEvalRun should handle empty testing results`() = runTest {
        // Arrange
        val evalRun = createSampleEvalRun()
        val eval = createSampleEval(evalRun.evalId)
        val completionMessages = listOf(ChatMessage("user", "text"))
        ...
        val jsonLines = listOf("{\"test\":\"data\"}")
        val generationResults = mapOf<Int, GenerationService.CompletionResult>() // Empty results
    ...
    @Test
    fun `processEvalRun should handle missing JSON line for an index during criteria evaluation`() = runTest {
        // Arrange
        val evalRun = createSampleEvalRun()
        val eval = createSampleEval(evalRun.evalId)
        val completionMessages = listOf(ChatMessage("user", "text"))
    ...
    @Test
    fun `processEvalRun should handle exception in final update and log error`() = runTest {
        // Arrange
        val evalRun = createSampleEvalRun()
        val eval = createSampleEval(evalRun.evalId)
    ...
    private fun createSampleEvalRun(
        ...
        dataSource: RunDataSource = CompletionsRunDataSource(
            source = FileDataSource("test.jsonl"),
            model = "gpt-4",
            inputMessages = TemplateInputMessages(
