...
class ChatCompletionsGenerationServiceTest {
    ...
    @Test
    fun `generateCompletions should process messages correctly when mock API returns valid response`() = runBlocking {
        ...
        val dataSourceConfig = CustomDataSourceConfig(
            schema = mapOf(
                ...
                "properties" to JsonValue.from(mapOf(
        ...
        val completionMessagesSet = mapOf(
            0 to messages
        )

        // Act
        val results = service.generateCompletions(
    ...
    @Test
    fun `generateCompletions should handle multiple completion message sets with errors`() = runBlocking {
        // Arrange
        val service = ChatCompletionsGenerationService()
