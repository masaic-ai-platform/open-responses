...
class EvalRunServiceTest {
    ...
    @Test
    fun `createEvalRun should create run with completions data source`() = runTest {
        // Given
        val evalId = "eval_123456"
        val eval = createSampleEval(evalId)
        val dataSource = CompletionsRunDataSource(
        ...
        coVerify { 
            ...
            evalRunRepository.createEvalRun(match {
                it.name == "Test Run" &&
                it.evalId == evalId &&
    ...
    @Test
    fun `createEvalRun should throw exception when eval does not exist`() = runTest {
        // Given
        val evalId = "non_existent_eval"
        val dataSource = mockk<CompletionsRunDataSource> {
    ...
    @Test
    fun `createEvalRun should throw exception when API key is missing`() = runTest {
        // Given
        val evalId = "eval_123456"
        val dataSource = mockk<CompletionsRunDataSource> {
            every { model } returns "gpt-4"
    ...
    @Test
    fun `getEvalRun should return evaluation run by ID`() = runTest {
        // Given
    ...
    @Test
    fun `getEvalRun should return null when run does not exist`() = runTest {
        // Given
    ...
    @Test
    fun `listEvalRunsByEvalId should return all runs for an eval`() = runTest {
        // Given
        val evalId = "eval_123456"
        val eval = createSampleEval(evalId)
        val runs = listOf(
    ...
    @Test
    fun `listEvalRunsByEvalId should throw exception when eval does not exist`() = runTest {
        // Given
        val evalId = "non_existent_eval"
    ...
    @Test
    fun `listEvalRunsByEvalId with params should return filtered runs`() = runTest {
        // Given
        val evalId = "eval_123456"
        val eval = createSampleEval(evalId)
        val runs = listOf(
        ...
        coEvery { 
            evalRunRepository.listEvalRunsByEvalId(
                evalId, after, limit, order, status
            ) 
        } returns runs
    ...
    @Test
    fun `listEvalRunsByEvalId with params should throw exception when eval does not exist`() = runTest {
        // Given
        val evalId = "non_existent_eval"
        val after = "run_0"
        val limit = 10
    ...
    @Test
    fun `deleteEvalRun should return true when run exists`() = runTest {
        // Given
        val evalRunId = "run_123456"
    ...
    @Test
    fun `deleteEvalRun should return false when run does not exist`() = runTest {
        // Given
        val evalRunId = "non_existent_run"
