...
class EvalRunControllerTest {
    ...
    fun setup() {
        ...
        headers = LinkedMultiValueMap<String, String>().apply {
            add("Authorization", "Bearer test-api-key")
        }
    }

    @Test
    fun `createEvalRun should return created evaluation run with status 201`() = runBlocking {
        // Arrange
        val evalId = "eval-123"
        val dataSource: RunDataSource = mockk<CompletionsRunDataSource>()
        val request = CreateEvalRunRequest(
            name = "Test Eval Run",
            dataSource = dataSource,
            metadata = mapOf("key" to "value")
        )

        val expectedEvalRun = EvalRun(
            apiKey = "test-api-key",
            id = "run-123",
            evalId = evalId,
            name = request.name,
    ...
    @Test
    fun `getEvalRun should return evaluation run when found`() = runBlocking {
        // Arrange
        val evalId = "eval-123"
        val runId = "run-123"
        val expectedEvalRun = EvalRun(
    ...
    @Test
    fun `getEvalRun should throw ResponseStatusException when not found`() = runBlocking {
        // Arrange
        val evalId = "eval-123"
    ...
    @Test
    fun `getEvalRun should throw ResponseStatusException when run does not belong to eval`() = runBlocking {
        // Arrange
        val evalId = "eval-123"
        val runId = "run-123"
    ...
    @Test
    fun `listEvalRuns should return list of evaluation runs`() = runBlocking {
        // Arrange
        val evalId = "eval-123"
        val evalRuns = listOf(
            EvalRun(
        ...
        // Act
        val response = evalRunController.listEvalRuns(
            evalId = evalId,
            after = null,
            limit = 20,
            order = "asc",
    ...
    @Test
    fun `listEvalRuns should filter runs by status`() = runBlocking {
        // Arrange
        val evalId = "eval-123"
    ...
    @Test
    fun `listEvalRuns should throw exception for invalid order`() = runBlocking {
        // Arrange
        val evalId = "eval-123"
    ...
    @Test
    fun `listEvalRuns should throw exception for invalid status`() = runBlocking {
        // Arrange
        val evalId = "eval-123"
    ...
    @Test
    fun `deleteEvalRun should return no content when successful`() = runBlocking {
        // Arrange
        val evalId = "eval-123"
        val runId = "run-123"
    ...
    @Test
    fun `deleteEvalRun should throw ResponseStatusException when run not found`() = runBlocking {
        // Arrange
        val evalId = "eval-123"
        val runId = "non-existent-id"
    ...
    @Test
    fun `deleteEvalRun should throw ResponseStatusException when run does not belong to eval`() = runBlocking {
        // Arrange
        val evalId = "eval-123"
        val runId = "run-123"
        val wrongEvalId = "eval-456"
    ...
    @Test
    fun `deleteEvalRun should throw ResponseStatusException when deletion fails`() = runBlocking {
        // Arrange
        val evalId = "eval-123"
        val runId = "run-123"
