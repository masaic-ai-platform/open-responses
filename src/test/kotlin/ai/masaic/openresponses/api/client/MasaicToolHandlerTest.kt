...
class MasaicToolHandlerTest {
    ...
    fun `handleMasaicToolCall(chatCompletion, params) - with a valid tool call should add functionCall and functionCallOutput`() {
        ...
        coEvery { toolService.executeTool("myToolFunction", "{\"key\":\"value\"}", any(), any()) } returns "Tool execution result"
    ...
    fun `handleMasaicToolCall(chatCompletion, params) - with tool execution error should propagate error and end span`() {
        ...
        coEvery { toolService.executeTool("errorTool", any(), any(), mockk()) } throws RuntimeException("Tool execution failed")
    ...
    fun `handleMasaicToolCall(params, response) - with a recognized function call should add functionCall and functionCallOutput`() {
        ...
        // Let’s pretend the toolService recognizes and executes "myToolFunction"
        every { toolService.getFunctionTool("myToolFunction") } returns mockk()
        coEvery { toolService.executeTool("myToolFunction", "{\"foo\":\"bar\"}", any(), any()) } returns "Executed tool"
    ...
    fun `handleMasaicToolCall(chatCompletion, params) - with multiple tool calls should process all`() {
        ...
        coEvery { toolService.executeTool("toolOne", "{\"param\":\"value1\"}", any(), any()) } returns "Result from tool one"
        coEvery { toolService.executeTool("toolTwo", "{\"param\":\"value2\"}", any(), any()) } returns "Result from tool two"
    ...
    fun `handleMasaicToolCall(chatCompletion, params) - with null tool result should skip adding output`() {
        ...
        coEvery { toolService.executeTool("nullResultTool", any(), any(), any()) } returns null
    ...
    fun `handleMasaicToolCall(params, response) - should append to existing response items`() {
        ...
        coEvery { toolService.executeTool("newFunction", "{\"param\":\"new\"}", any(), any()) } returns "New function result"
    ...
    fun `handleMasaicToolCall - should properly record all required telemetry attributes`() {
        ...
        coEvery { toolService.executeTool("telemetryTool", any(), any(), any()) } returns "Telemetry result"
    ...
    fun `handleMasaicToolCall(chatCompletion, params) - with both text content and tool calls`() {
        ...
        coEvery { toolService.executeTool("mixedContentTool", "{\"param\":\"mixed\"}", any(), any()) } returns "Mixed result"
    ...
    fun `handleMasaicToolCall(chatCompletion, params) - with parent observation passed explicitly`() {
        ...
        coEvery { toolService.executeTool("parentObsTool", "{\"param\":\"parentTest\"}", any(), any()) } returns "Parent observation test result"
    ...
    fun `handleMasaicToolCall(chatCompletion, params) - with malformed tool arguments should propagate error`() {
        ...
        coEvery { toolService.executeTool("malformedArgsTool", "{invalid json", any(), any()) } throws IllegalArgumentException("Malformed JSON arguments")
    ...
    fun `handleMasaicToolCall(params, response) - with response as input should preserve existing items`() {
        ...
        coEvery { toolService.executeTool("preserveFunction", "{\"preserve\":\"existing\"}", any(), any()) } returns "Preserved result"
