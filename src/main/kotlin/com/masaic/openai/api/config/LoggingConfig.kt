...
class LoggingConfig {
    ...
    @Bean
    fun requestIdFilter(): WebFilter {
        return RequestTraceWebFilter()
    }
    ...
    class RequestTraceWebFilter : WebFilter {
        override fun filter(exchange: ServerWebExchange, chain: WebFilterChain): Mono<Void> {
            ...
            val spanId = existingSpanId ?: UUID.randomUUID().toString().replace("-", "").substring(0, 16)
            ...
            logger.info {
                ...
                "client: ${exchange.request.remoteAddress?.address?.hostAddress ?: "unknown"}, " +
                "traceId: $traceId"
            }
