...
class MasaicStreamingService(
    ...
    fun createCompletionStream(
        ...
        while (shouldContinue) {
            ...
            val iterationResult = executeStreamingIteration(
                client,
                currentParams,
                responseId,
            ...
            subscription.subscribe { completion ->
                if (!completion._choices().isMissing()) {
                    ...
                    if (completion.choices().any { choice ->
                            choice.finishReason().isPresent && listOf("stop", "length", "content_filter")
                        ...
                        val finishReason = completion.choices().find { it.finishReason().isPresent }?.finishReason()?.get()?.asString()
                        when (finishReason) {
                            "stop" -> {
                            ...
                            "length", "content_filter" -> {
                                val incompleteDetails = if (finishReason == "length") {
                        ...
                        if (completion.choices().any { choice ->
                            ...
                            if (internalToolItemIds.isEmpty()) {
                                ...
                                updatedParams = params.toBuilder()
    ...
    private fun buildInitialResponseItems(initialParams: ResponseCreateParams): MutableList<ResponseInputItem> {
        return if (initialParams.input().isResponse()) {
     ...
     */
    private fun tooManyToolCalls(inputItems: List<ResponseInputItem>): Boolean =
        inputItems.count { it.isFunctionCall() } > allowedMaxToolCalls
    ...
    private fun ProducerScope<ServerSentEvent<String>>.handleTextCompletion(
        ...
        if (textAccumulator.isNotEmpty()) {
            ...
            val singleMessage = ResponseOutputItem.ofMessage(
    ...
    private fun ProducerScope<ServerSentEvent<String>>.handleFunctionCallDone(
        ...
        functionCallAccumulator.forEach { (key, events) ->
            ...
            val alreadyAdded = responseOutputItemAccumulator
