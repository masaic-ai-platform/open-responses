...
class DomainLabellingController(
    ...
    suspend fun embed(
        ...
        domainLabelSuggester.createEmbeddings(
            listOf(
    ...
    suspend fun getGenericLabels(): List<GenericLabel> = (conversationRepository as MongoConversationRepository).getGenericLabels("generic")

    @GetMapping("/buckets/labels/domain")
    suspend fun getDomainLabels(): List<Rule> = ruleRepository.getDistinctRules()

    @PostMapping("/buckets/labels/suggest", produces = [MediaType.APPLICATION_JSON_VALUE])
    suspend fun suggest(
        @RequestBody request: SuggestLabelsRequest,
        @RequestHeader("Authorization") authHeader: String,
    ): Map<Int, SuggestedLabelResponse> = domainLabelSuggester.suggest(request.copy(apiKey = authHeader))

    @PostMapping("/labels/accept", produces = [MediaType.APPLICATION_JSON_VALUE])
    suspend fun acceptLabel(
        @RequestBody request: AcceptLabelRequest,
        @RequestHeader("Authorization") authHeader: String,
