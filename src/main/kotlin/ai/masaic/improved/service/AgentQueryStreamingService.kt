...
class AgentQueryStreamingService(
     ...
     */
    suspend fun processQueryStream(request: AgentQueryRequest, apiKey: String): Flow<ServerSentEvent<String>> = flow {
        ...
        try {
            ...
            while (retryCount <= request.maxRetries) {
                try {
                    ...
                    emit(createEvent(AgentResponseEvent.CypherGeneration(
                    ...
                    if (executionResult.success) {
                        ...
                        if (request.enableVisualization && results.isNotEmpty()) {
                            logger.info { "Visualization enabled and results not empty. Starting visualization generation..." }
                            emit(createEvent(AgentResponseEvent.VisualizationGeneration("Analyzing data for visualization...")))
                            emit(createEvent(AgentResponseEvent.Progress("Creating Visualization", 0.9f, "Generating chart recommendations")))
                            
                            try {
                                logger.info { "Calling visualizationService.analyzeForVisualization..." }
                                ...
                                logger.info { "Received recommendation: shouldVisualize=${recommendation.shouldVisualize}, chartType=${recommendation.chartType}" }
                                
                                if (recommendation.shouldVisualize) {
                                    logger.info { "Recommendation says to visualize. Calling generateVisualization..." }
                                    visualization = visualizationService.generateVisualization(
                                        recommendation,
                                        results,
                                        request.query,
                                        apiKey
                                    ...
                                    logger.info { "Generated visualization: ${if (visualization != null) "SUCCESS" else "NULL"}" }
                                    ...
                                    if (visualization != null) {
                                        ...
                                        logger.warn { "Visualization generation returned null despite recommendation.shouldVisualize=true" }
                                    }
                                } else {
                                    logger.info { "Recommendation says NOT to visualize. Reason: ${recommendation.reasoning}" }
                                }
                            } catch (e: Exception) {
                                logger.warn(e) { "Error generating visualization, continuing without it: ${e.message}" }
                            }
                        } else {
                        ...
                        finalResponse = AgentQueryResponse(
